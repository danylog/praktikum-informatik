...
Fahrzeug::Fahrzeug() : p_iID(++p_iMaxID), p_sName(""), p_dMaxGeschwindigkeit(0.0) {
...
os << setw(4) << p_iID << " "
		...
		<< setw(15) << setprecision(2) << dMaxGeschwindigkeit() << " "
...
ostream& operator<<(ostream& os, Fahrzeug& f) { //<<operator (uberladet)
    f.vAusgeben(os);
    return os;
}
